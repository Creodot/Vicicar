...
const textSplit = ({ widget }) => {
  ...
  const initTextSplit = () => {
    ...
    splitText = new SplitText(widget, {
      type: "chars,words,lines",
      linesClass: "line",
      wordsClass: "word",
      charsClass: "char",
      ...widget.hasAttribute("data-line-animation") && { mask: "lines" }
    });
  ...
  const startAnimationWhenVisible = () => {
    ...
    gsapWithCSS.fromTo(
      ...
      {
        y: "150%",
        opacity: Number(widget.getAttribute("data-start-opacity")) || 0,
        rotationZ: 1,
      ...
      },
      {
        ...
        opacity: 1,
        rotationZ: 0,
        duration: widget.getAttribute("data-duration") ? Number(widget.getAttribute("data-duration")) : 0.5,
        stagger: {
          amount: widget.getAttribute("data-stagger") ? Number(widget.getAttribute("data-stagger")) : splitText.chars.length / 10
        },
        ease: "power2.out",
        scrollTrigger: {
          trigger: widget,
        ...
        onUpdate: function() {
          if (this.progress() >= 0.5 && widget.getAttribute("data-class-removed") !== "true") {
            widget.setAttribute("data-class-removed", "true");
            if (widget.closest("[data-marquee-heading]")) {
              const marqueeParent = widget.closest("[data-marquee-heading]");
              marqueeParent.classList.remove(CLASSES.DISABLED);
            }
          }
        }
      }
...
const WIDGET_SELECTOR = '[data-widget="text-split"]';
const handleSplittingInitialized = () => {
...
export {
  handleSplittingInitialized as h
};
